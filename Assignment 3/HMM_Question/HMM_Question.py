...
def emission_probability(state, observation,sigma):
    ...
    # Extract the state's position and observation coordinates
    state_x, state_y = state[0]
    obs_x, obs_y = observation

    # Calculate the squared Euclidean distance between the state and observation
    distance_squared = (state_x - obs_x) ** 2 + (state_y - obs_y) ** 2

    # Calculate the log emission probability using the Gaussian distribution
    log_emission_prob = - (0.5 * np.log(2 * np.pi * sigma ** 2)) - (0.5 * distance_squared / (sigma ** 2))
...
def transition_probability(prev_state, curr_state, movement_policy):
    ...
    # Check if the previous state is an obstacle
    if(is_obstacle(prev_state[0])): 
        return float('-inf')

    if movement_policy == 'straight_until_obstacle':
        possible_new_states =  straight_until_obstacle_possible_states(prev_state)
    elif movement_policy == 'random_walk':
        possible_new_states =  random_walk_possible_states(prev_state)
    else:
        raise ValueError("Unknown movement policy")

    if curr_state in possible_new_states:
        prob = 1 / len(possible_new_states) # uniform probabilities for possible states
        return np.log(prob)
    else:
        return float('-inf') # transition is impossible # probability is 0
...
def viterbi(observations, start_state, movement_policy,states,sigma):
    ...
    # at t = 2, 3, ... T
    for t in tqdm(range(2, T), desc="Viterbi Algorithm forward messaging"):
        V_t = {}
        path_t = {}
        for curr_state in states:
            # Find the maximum probability and corresponding state for the current state
            ...
            # Update the probability and path
    ...
    # Identify the most probable final state
